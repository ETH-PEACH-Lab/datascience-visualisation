...
const NotebookSelector: React.FC<NotebookSelectorProps> = ({
  ...
  const handleRemoveNotebook = (notebookId: number) => {
    const newSelectedNotebooks = selectedNotebooks.filter(
      id => id !== notebookId
    );
  ...
  const handleAddNotebook = () => {
    const notebookId =
      selectedValue === 'All Notebooks'
        ? -2
        : parseInt(selectedValue.match(/\d+/)?.[0] || '', 10);
    if (notebookId === -2) {
      onSelectionChange([-2]); // If "ALL" is selected, clear other selections and set "ALL"
    } else if (
      !selectedNotebooks.includes(notebookId) &&
      notebookIds.includes(notebookId)
    ) {
      const newSelectedNotebooks = [...selectedNotebooks, notebookId].filter(
        id => id !== -2
      ); // Remove "ALL" (-2) before adding a specific notebook
      onSelectionChange(newSelectedNotebooks);
    }
    ...
    <div className="selector-container">
      ...
      <div className="selected-elements" id="selected-elements">
        {selectedNotebooks.map(notebookId => (
          <div key={notebookId} className="element">
            {notebookId === -2
              ? 'All Notebooks'
              : `Notebook ${notebookId}: ${notebookNames[notebookIds.indexOf(notebookId)]}`}
            {notebookId !== -2 && (
              <>
                <button
                  className="remove-button"
                  onClick={() => handleRemoveNotebook(notebookId)}
                >
                  Remove
                </button>
                <button
                  ...
                  onClick={() =>
                    handleOpenNotebook(
                      notebookNames[notebookIds.indexOf(notebookId)],
                      competition_name
                    )
                  }
                >
      ...
      <datalist id="elements">
        {notebookIds.map(id => (
          <option
            key={id}
            value={id === -2 ? 'All Notebooks' : `Notebook ${id}`}
          >
